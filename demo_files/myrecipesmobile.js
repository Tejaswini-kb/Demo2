var s_account = "timemyrecipesnew";
var s_time_rsid2 = "timeglobalvideo";
if (/(file:\/\/\/|localhost|qa-esearch|dev-esearch|dev\.myrecipes\.com|qa\.myrecipes\.com|v5\.timeinc\.net)/.test(location.href))
{
    s_account = "timemyrecipesnewdev";
    s_time_rsid2 = "timeglobalvideodev";
}
...
s_time.linkInternalFilters = "javascript:,myrecipes.com,cheetahmail.com,timeinc.net,venturaassociates.net,usscpromotions.com,localhost";
...
s_time.omniLogin = false;
s_time.origRefExp = new Function("" + "var tiiDate=new Date(); tiiDate.setTime(tiiDate.getTime()-(24*60*60*1000)); return tiiDate.toGMTString()");
if (s_time.c_r("orig_ref") && s_time.c_r("orig_ref") != "") {
    s_time.referrer = s_time.c_r("orig_ref");
    document.cookie = "orig_ref=; expires=" + s_time.origRefExp() + "; path=/";
}
s_time.usePlugins = true;

function s_time_doPlugins(s_time) {
    s_time.mobile = true;
    s_time.channel = "myrecipes mobile";
    if (!s_time.campaign && s_time.campaign != "") {
        s_time.campaign = s_time.getQueryParam("xid,fb_ref");
    }
    if (!s_time.eVar1 && s_time.eVar1 != "") {
        s_time.eVar1 = s_time.getQueryParam("iid");
    }
    if (!s_time.eVar2 && s_time.eVar2 != "") {
        s_time.eVar2 = s_time.getQueryParam("pkw");
    }
    s_time.eVar2 = s_time.getValOnce(s_time.eVar2, "s_var_2", 0);
    if (!s_time.eVar14 && s_time.eVar14 != "") {
        s_time.eVar14 = s_time.getQueryParam("pid");
    }
    s_time.eVar23 = "D=pageName";
    s_time.eVar24 = "D=c16";
    s_time.eVar27 = "D=c20";
    s_time.eVar28 = "D=c11";
    s_time.eVar13 = "D=User-Agent";
    s_time.prop17 = location.href;
    s_time.eVar35 = "D=c17";
    s_time.prop47 = "H.24.3-2015.06.02";
    if (s_time.prop18) {
        ...
        if (t_search) {
            s_time.events = s_time.events + ",event34";
        }
    }
    if (s_time.c_r("auth_provider")) {
        s_time.prop31 = "logged-in: " + s_time.c_r("auth_provider");
        s_time.omniLogin = true;
    } else {
        s_time.prop31 = "non-logged-in";
        s_time.omniLogin = false;
    }
    s_time.eVar40 = "D=c31";
    var omniRegC = "omni_registered";
    var omniRegC_exp = new Date();
    var omniRegC_ct = omniRegC_exp.getTime();
    omniRegC_exp.setTime(omniRegC_ct + 5 * 365 * 24 * 60 * 60 * 1000);
    if (s_time.events.indexOf("event21") > -1) {
        s_time.c_w(omniRegC, "reg", omniRegC_exp);
    }
    if (s_time.omniLogin && !s_time.c_r(omniRegC)) {
        s_time.c_w(omniRegC, "reg", omniRegC_exp);
    }
    s_time.c_r(omniRegC) ? s_time.eVar46 = "registered" : s_time.eVar46 = "non-registered";
    ...
    for (key in allExp) {
        if (allExp.hasOwnProperty(key) && allExp[key].hasOwnProperty("enabled") && !allExp[key].site_catalyst_evar) {
    ...
    if (s_time.pev3 == "video") {
        s_time.sa(s_account2);
    }
}
...
if (s_time.enableVideoTracking) {
    ...
    s_time.Media.trackVars = "events,eVar6,eVar7,eVar8,prop48,eVar53,eVar56,eVar57,eVar58,eVar59,channel,server,contextData.bc_tags,contextData.bc_channel";
...
s_time.comscoreCust = "6035728";
s_time.comscoreId = "Myrecipes.com Mobile";
s_time.nielsenCust = "us-100120";
s_time.nielsenSite = "c10";

//MRDRPL-961 - Functions mr_TemplateLoaded and mr_TemplateReady are the customization done to the BC adpolicy and omniture tracking (event9) - Aravindan N
function mr_TemplateLoaded(experienceID) {
    player = brightcove.api.getExperience(experienceID);
    modVP = player.getModule(brightcove.api.modules.APIModules.VIDEO_PLAYER);
    modExp = player.getModule(brightcove.api.modules.APIModules.EXPERIENCE);
    modAdPolicy = player.getModule(brightcove.api.modules.APIModules.ADVERTISING);
    // ads vars
    var adPolicy = new Object();
    var overlayAdzone = "video_bc/" + adzone.split('/')[0] + "_bc";
    adPolicy.adServerURL = "http://pubads.g.doubleclick.net/gampad/ads?sz=1000x1&iu=/8484/mre/" + overlayAdzone + "&ciu_szs=300x250&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&url=[referrer_url]&correlator=[timestamp]";
    adPolicy.prerollAds = true;
    var currentVideoOverlayAdKeyValuePairs = currentOverlayVideoData.video.adkeyvaluepairs || '';
    adPolicy.prerollAdKeys = currentVideoOverlayAdKeyValuePairs.replace(/\|/g,';').replace(/\s/g,'%20');
    modAdPolicy.setAdPolicy(adPolicy);
    modExp.addEventListener(brightcove.api.events.ExperienceEvent.TEMPLATE_READY, mr_TemplateReady);
};

mr_TemplateReady  = function(event){
	...
	modVP.addEventListener(brightcove.api.events.MediaEvent.COMPLETE, onComplete); /* Nielsen Complete */
	modAdPolicy.addEventListener(brightcove.api.events.AdEvent.START, onAdStart); /* comScore Ad Starts */
};
...
function onTemplateReady(evt) {
    ...
    modAd.addEventListener(brightcove.api.events.AdEvent.START, onAdStart);
}
...
function omniAdTrack() {
    ...
    (s_time.channel && s_time.prop16) ? s_time.contextData["bc_channel"] = s_time.channel + ": " + s_time.prop16: s_time.contextData["bc_channel"] = "myrecipes mobile: unknown";
...
function onPause(evt) {
    s_time.omniPauseFlg = true;
}

function onAdStart(evt) {
    var cs_adpix = new Image();
    cs_adpix.src = "http://b.scorecardresearch.com/p?c1=1&c2=" + s_time.comscoreCust + "&c3=" + s_time.comscoreId + "&c5=010000&rn=" + Math.ceil(Math.random() * 10000000000);
    omniAdTrack();
}

function onComplete(evt) {
    s_time.omniPauseFlg = false;
    var scDU = Math.round(evt.duration);
    var scCI = s_time.nielsenCust;
    var scTL = escape(mediaName);
    var scCG = escape(mediaName);
    var scC6 = s_time.nielsenSite;
    var vc_pix = new Image();
    vc_pix.src = "http://secure-us.imrworldwide.com/cgi-bin/m?ci=" + scCI + "&cg=" + scCG + "&tl=dav2-" + scTL + "&du=" + scDU + "&cc=1&c6=vc," + scC6 + "&rnd=" + Math.ceil(Math.random() * 100000000);
}

function onPlay(evt) {
    mediaLength = evt.duration;
    mediaOffset = Math.floor(evt.position);
    if (s_time.mediaChange) {
        mediaOffset = 0;
		checkForFifteen = 0;
		checkForThirty = 0;
    }
    ...
    var videoMM = Math.floor(mediaLength / 60).toString();
    var videoSS = Math.floor(mediaLength % 60).toString();
    (videoMM.length == 1) ? videoMM = "(0" + videoMM: videoMM = "(" + videoMM;
    (videoSS.length == 1) ? videoSS = "0" + videoSS + ")": videoSS = videoSS + ")";
    (s_time.prop16) ? mediaPrefix = s_time.prop16: mediaPrefix = "unknown";
    mediaName = mediaPrefix + ": " + mediaFriendly + " " + videoMM + ":" + videoSS;
    ...
    if (mediaOffset == 0) {
        ...
        (s_time.channel && s_time.prop16) ? s_time.contextData["bc_channel"] = s_time.channel + ": " + s_time.prop16: s_time.contextData["bc_channel"] = "myrecipes mobile: unknown";
        media_franchise = "franchise unavailable";
		media_campaign_id = "campaign id unavailable";
		media_partner = "partner unavailable";
		...
		if (typeof media_customFields.campaign_id == "string") {
		media_campaign_id = media_customFields.campaign_id.toLowerCase();
		}
		if (typeof media_customFields.partner == "string") {
		media_partner = media_customFields.partner.toLowerCase();
		}
		s_time.eVar7 = media_partner;
		s_time.eVar8 = media_campaign_id;
        s_time.eVar53 = mediaID;
        s_time.eVar57 = media_franchise;
        s_time.eVar58 = mediaPlayerName;
        s_time.eVar59 = mediaPlayerType;
        ...
        var scC6 = s_time.nielsenSite;
        var vc_pix = new Image();
        vc_pix.src = "http://secure-us.imrworldwide.com/cgi-bin/m?ci=" + scCI + "&cg=" + scCG + "&tl=dav0-" + scTL + "&cc=1&c6=vc," + scC6 + "&rnd=" + Math.ceil(Math.random() * 100000000);
        var cs_pix = new Image();
        cs_pix.src = "http://b.scorecardresearch.com/p?c1=1&c2=" + s_time.comscoreCust + "&c3=" + s_time.comscoreId + "&c5=020000&rn=" + Math.ceil(Math.random() * 10000000000);
        s_time.omniPauseFlg = true;
    }
}
...
if(evt.position>=15 && checkForFifteen==0)
  {
    ...
    (s_time.channel && s_time.prop16) ? s_time.contextData['bc_channel']=s_time.channel+': '+s_time.prop16 : s_time.contextData['bc_channel']='myrecipes: unknown';
